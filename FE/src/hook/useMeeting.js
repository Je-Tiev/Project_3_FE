...
export function useMeetingWithWebRTC(meetingId) {
  ...
  const createPeerConnection = useCallback((connectionId, stream) => {
    ...
    pc.onicecandidate = (evt) => {
      if (evt.candidate && connectionRef.current?.state === HubConnectionState.Connected) {
        try {
          connectionRef.current.invoke("SendSignal", connectionId, { type: "ice", candidate: evt.candidate });
        } catch (e) {
  ...
  const handleSignal = useCallback(async (fromConnectionId, signal) => {
    ...
    try {
      if (signal.type === "offer") {
        ...
        connectionRef.current?.invoke("SendSignal", fromConnectionId, { type: "answer", sdp: pc.localDescription });
  ...
  const handleExistingParticipants = useCallback(async (existingParticipants) => {
    ...
    for (const p of existingParticipants) {
      if (!peersRef.current.has(p.connectionId)) {
        ...
        try {
          ...
          connectionRef.current?.invoke("SendSignal", p.connectionId, { type: "offer", sdp: pc.localDescription });
        } catch (e) {
  ...
  const cleanup = useCallback(async () => {
    ...
    if (connectionRef.current && connectionRef.current.state === HubConnectionState.Connected) {
      try { await connectionRef.current.stop(); } catch (e) { console.warn(e); }
    }
 ...
 const joinRoom = useCallback(async (id) => {
  ...
  try {
    ...
    await connectionRef.current.invoke("JoinRoom", id);
  ...
  const registerSignalREvents = useCallback((connection) => {
      ...
      if (localStream) {
        if (!peersRef.current.has(p.connectionId)) {
          ...
          pc.createOffer()
            ...
            .then(() => connectionRef.current?.invoke("SendSignal", p.connectionId, { type: "offer", sdp: pc.localDescription }))
    ...
    connection.on("UserLeft", (p) => {
      setParticipants(prev => prev.filter(x => x.connectionId !== p.connectionId));
      const entry = peersRef.current.get(p.connectionId);
      if (entry) entry.pc.close();
      peersRef.current.delete(p.connectionId);
      setRemoteStreams(prev => { const copy = { ...prev }; delete copy[p.connectionId]; return copy; });
    });
  ...
  }, [createPeerConnection, handleExistingParticipants, handleSignal, joinRoom, meetingId]);
  ...
  useEffect(() => {
    ...
    connection.start()
      ...
      .catch(err => { console.error(err); setError("Không thể kết nối server"); setStatus("error"); });

    const handleUnload = () => { isUnmountingRef.current = true; cleanup(); };
  ...
  const sendMessage = async (text) => connectionRef.current?.invoke("SendMessage", meetingId, text).catch(console.warn);
  const startScreenShare = async () => connectionRef.current?.invoke("StartScreenShare", meetingId).catch(console.warn);
  const stopScreenShare = async () => connectionRef.current?.invoke("StopScreenShare", meetingId).catch(console.warn);

  return {
    status, participants, remoteStreams, messages, error,
    localStream: localStreamRef.current,
    roomName, isMicOn, isVideoOn,
    toggleCamera, toggleMicrophone, startScreenShare, stopScreenShare, sendMessage, ensureLocalMedia
  };
}
